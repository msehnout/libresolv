...
#[test]
fn test_parse_dns_question_name() {
    let input = vec![0x03, 0x6e, 0x69, 0x63, 0x02, 0x63, 0x7a, 0x00, 0x00, 0x01, 0x00, 0x01];
    let output = msg::parse_dns_question(&input[..]).unwrap().1;
    assert_eq!(output.1, vec![
    msg::NameUnit::Label(String::from("nic")),
    msg::NameUnit::Label(String::from("cz")),
    msg::NameUnit::End,
    ]);
    // assert_eq!(output.0.qtype, 1);
    // assert_eq!(output.0.class, 1);
}
...
fn test_parse_dns_rr() {
    ...
    let (_, name, rdata) = msg::parse_dns_rr(&input[..]).unwrap().1;
    assert_eq!(name, vec![
    msg::NameUnit::Label(String::from("example")),
    msg::NameUnit::Label(String::from("com")),
    msg::NameUnit::End,
    ]);
    let mut addr = [0u8; 4];
    addr.copy_from_slice(&rdata[0..4]);
    assert_eq!(Ipv4Addr::from(addr), Ipv4Addr::from_str("93.184.216.34").unwrap());
}
...
fn test_parse_query() {
    ...
    // println!("{:?}", q);
    assert_eq!(q.question[0].name, "nic.cz.".to_string());
}
